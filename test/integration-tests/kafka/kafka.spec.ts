 ...
 *
 * Runs end-to-end validation of producers (sensors) and consumer (IoT Manager)
 * across two scenarios:
 *   1. Non-compressed messages
 *   2. Compressed messages (gzip)
 *
 * Test flow (per scenario):
 * - Init: verify sensors & IoT Manager are stopped, clear topic if needed.
 * - Producers: start both sensors, publish 100k msgs each, then auto-stop.
 * - Topic: validate telemetry topic metadata (2 partitions).
 ...
 * - Consumers (2 instances): restart via REST, consume until lag=0, stop, record time.
 * - Compare: log/compare consumption times (1 vs 2 consumers).
 *
 * REST API coverage:
 * - publisher: /api/start, /api/stop, /api/stats, /api/reset-stats
 * - consumer: /api/start, /api/stop, /api/stats, /api/reset-offsets,
 *             /api/delete-messages, /api/topic/:topic
 *
 * Goals:
 * - Validate end-to-end Kafka pipeline across containers.
 * - Ensure topic partitioning, offsets, lag tracking, and resets work correctly.
 * - Compare message publishing with and without compression.
 * - Measure scaling effect (1 vs 2 consumers).
 * - Provide control & observability via REST endpoints.
 */
...
for (const scenario of scenarios) {
  test.describe(`Test Kafka producers and consumers: ${scenario.description}`, () => {
    ...
    test('1. Ensure there are no messages published initially', async ({ playwright }) => {
      testSummary.push(`   Scenario: ${scenario.description}`);
      ...
      if (topicMessages > 0) {
        ...
        const deleteResponse = await iotManagerContext.post('/api/delete-messages');
    ...
    test('4. Produce telemetry data and ensure messages are published and sensors are stopped after that', async ({ playwright }) => {
      ...
      const sensor2Stat = await getServiceStat(sensor2Context);
      expect(sensor1Stat).toHaveProperty('serviceStatus', 'stopped');
      expect(sensor1Stat.messagesPublished).toBeGreaterThanOrEqual(MESSAGES_PER_SENSOR);
      expect(sensor1Stat).toHaveProperty('stoppedAt');
      expect(sensor2Stat).toHaveProperty('serviceStatus', 'stopped');
      expect(sensor2Stat.messagesPublished).toBeGreaterThanOrEqual(MESSAGES_PER_SENSOR);
    ...
    test('9. Reset consumer group offsets', async ({ playwright }) => {
      ...
      const iotManagerStat = await getServiceStat(iotManagerContext);
      expect(iotManagerStat.totalLag).toBeGreaterThanOrEqual(MESSAGES_PER_SENSOR * 2);
    });
...
test.afterAll(() => {
  ...
  fs.writeFileSync('test-reports/test-summary.txt', output);
  console.log('\n==== SHORT SUMMARY ====\n' + output + '\n=======================');
});
